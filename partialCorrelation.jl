...
n = 400  # number of rows (observations)
p = 20   # number of columns (variables)

# Julia code
Y = randn(n, 1);  # Y is n x 1
X = randn(n, p);  # X is n x p

df = DataFrame(X, :auto)
CSV.write("/data/miraldiNB/Katko/Projects/Julia2/Testing/fakeX.txt", df, delim='\t')
...
using LinearAlgebra 
function partialCorrelationMat(X::Matrix{Float64}; epsilon = 1e-5,  first_vs_all = false)
